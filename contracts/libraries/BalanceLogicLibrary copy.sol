...
library BalanceLogicLibrary {
    ...
    function getPastUserPointIndex(
        ...
        mapping(uint256 => IVotingEscrow.UserPoint[]) storage _userPointHistory,
        uint256 _tokenId,
        uint256 _timestamp
    ...
    function balanceOfNFTAt(
        ...
        if (lastPoint.permanent != 0) {
            ...
            lastPoint.bias -= lastPoint.slope * int256(_t - lastPoint.ts).toInt128();
            if (lastPoint.bias < 0) {
                lastPoint.bias = 0;
            }
    ...
    function supplyAt(
        ...
        for (uint256 i = 0; i < 255; ++i) {
            ...
            bias -= slope * int256(t_i - ts).toInt128();
            if (t_i == _t) {
                break;
            }
            slope += dSlope;
